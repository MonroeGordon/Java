...
/**
 * MinimizeButton creates and controls the maximize button at the top right of the window. This button causes the window to be minimized or restored to its previous size.
 * @author Monroe Gordon
 * @since 5/28/2022
 */
public class MinimizeButton extends JButton implements MouseListener {

	/** MinimizeButton serial version ID value. */
	private static final long serialVersionUID = -7790390600464041763L;
	
	/** Minimize button height value. */
	public static final int BUTTON_HEIGHT = 25 - ChessFrame.FRAME_BORDER_SIZE;
	/** Minimize button width value. */
	...
	/**
	 * Default constructor for the MinimizeButton that creates the maximize button.
	 */
	...
	public void paintComponent(Graphics g) {
		...
		if(rollover) {
			...
			if(pressed) {
				//Create translucent lighter title bar colored background
				g2.setBackground(new Color(128, 128, 196, 196));
				g2.clearRect(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT);
				
				//Draw translucent line
				g2.setColor(new Color(255, 255, 255, 128));
				g2.setStroke(new BasicStroke(2));
				g2.drawLine(21, 11, 30, 11);
			}
			//If the button is not pressed
			else {
				//Create lighter title bar colored background
