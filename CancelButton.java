...
/**
 * CancelButton creates and controls the cancel button in the new game dialog. This button causes a new chess game to start.
...
public class CancelButton extends JButton implements MouseListener {
	...
	/**
	 * Default constructor for the CancelButton that creates the exit button.
	 */
	public CancelButton() {
	...
	public void paintComponent(Graphics g) {
		...
		if(rollover) {
			...
			if(pressed) {
				...
				//Draw translucent Cancel string
				g2.setColor(new Color(255, 255, 255, 128));
				g2.setFont(new Font("Arial", Font.PLAIN, 24));
				FontMetrics metrics = g2.getFontMetrics();
				int strwidth = metrics.charsWidth("Cancel".toCharArray(), 0, "Cancel".length());
				int strheight = metrics.getHeight();
				g2.drawString("Cancel", (getWidth() / 2) - (strwidth / 2), getHeight() - (strheight / 2));
			}
